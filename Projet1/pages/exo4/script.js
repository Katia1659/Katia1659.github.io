...
const creerCarteFruit = (obj) => {
    ...
    boutonAcheter.addEventListener("click", () => {
        let total = parseFloat(inputQuantite.value * obj.prix);
...
searchBar.addEventListener("keyup", () => {
    ...
    const fruitsFiltrés = fruits.filter(fruit => fruit.nom.toLowerCase().includes(searchText.toLowerCase()));
    console.log(fruitsFiltrés);
    afficherFruits(fruitsFiltrés);
});

// filtre par prix:

let prixMinInput = document.getElementById('prix-min');
let prixMaxInput = document.getElementById('prix-max');

const boutonFiltre = document.getElementById("filtrer-prix");

boutonFiltre.addEventListener('click', () => {
    const prixMin = prixMinInput.value ? parseFloat(prixMinInput.value) : 0;
    const prixMax = prixMaxInput.value ? parseFloat(prixMaxInput.value) : Infinity;

    const fruitsFiltrés = fruits.filter(fruit => fruit.prix >= prixMin && fruit.prix <= prixMax);
    console.log(fruitsFiltrés);
